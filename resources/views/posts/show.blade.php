...
@section('content')
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-semibold text-slate-800">Detail {{ ucfirst('posts') }}</h1>
    ...
    <div class="flex items-center gap-2">
      ...
      @if (method_exists($post, 'trashed'))
        @if ($post->trashed())
          <form method="POST" action="{{ route('cms.posts.restore', $post->id) }}" class="inline">
            @csrf
            <button type="submit"
  ...
  <div class="bg-white rounded-xl shadow-sm ring-1 ring-slate-200 p-6">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
      <div>
        ...
        <dd class="mt-1 text-slate-900">{{ $post->id }}</dd>
      </div>
